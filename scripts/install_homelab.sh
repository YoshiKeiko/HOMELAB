...
# This script will OVERWRITE existing installations
################################################################################
...
# Optional tokens (leave empty if not needed)
...
################################################################################
# Phase 0: Cleanup Existing Installation
################################################################################

cleanup_existing() {
    log_section "Phase 0: Cleaning Up Existing Installation"
...
create_directory_structure() {
    log_section "Phase 1: Creating Complete Directory Structure"
    
    mkdir -p "$HOMELAB_DIR"/{Docker/{Data,Compose},isos,backups,docs,scripts}
    
    # Core Infrastructure
    mkdir -p "$DOCKER_DATA_DIR"/{
        traefik/{config,acme,logs},
    ...
    # Media Services
    mkdir -p "$DOCKER_DATA_DIR"/{
        plex/{config,transcode,media/{movies,tv,music,photos}},
        ...
        tautulli/config,
        organizr/config,
        transmission/{config,downloads,watch},
        qbittorrent/{config,downloads},
        sabnzbd/config,
        nzbget/config,
        jackett/config,
        tdarr/{server,configs,logs}
    }
    ...
    mkdir -p "$DOCKER_DATA_DIR"/{
        ...
        openvpn/config,
        rustdesk/data,
        guacamole/{postgresql,config,drive,record}
    }
    ...
    # Storage & Backup
    mkdir -p "$DOCKER_DATA_DIR"/{
        nextcloud/{data,config,apps,themes},
    ...
    mkdir -p "$DOCKER_DATA_DIR"/{
        matrix/{data,media},
        element/config,
        jitsi/{config,transcripts,prosody,jicofo,jvb},
        mattermost/{config,data,logs},
        rocketchat/{data,uploads}
    }
    
    # Utilities
    mkdir -p "$DOCKER_DATA_DIR"/{
        paperless/{data,media,consume},
        photoprism/{storage,originals,import},
        immich/{upload,library,thumbs,profile},
        calibre/{config,books},
        calibre-web/config,
        freshrss/{data,extensions},
        wallabag/data,
        bookstack/{data,uploads},
        heimdall/config,
        homer/assets,
        dashy/config
    }
...
install_dependencies() {
    log_section "Phase 2: Installing System Dependencies"
    ...
    brew install wget curl git htop jq yq openssl 2>/dev/null || true
    
    # Verify OrbStack is running
    if ! docker ps &> /dev/null; then
        log_warn "Please start OrbStack and press Enter to continue..."
        read
    fi
...
################################################################################
# Phase 3: Docker Networks
################################################################################

create_docker_networks() {
    log_section "Phase 3: Creating Docker Networks"
...
deploy_databases() {
    log_section "Phase 4: Deploying Database Stack"
...
services:
  ...
  influxdb:
    ...
    environment:
      ...
      INFLUXDB_ADMIN_PASSWORD: ${POSTGRES_PASSWORD}
    volumes:
      - $DOCKER_DATA_DIR/influxdb/data:/var/lib/influxdb
    networks:
      - database-net
    ports:
      - "8086:8086"
...
deploy_core_infrastructure() {
    log_section "Phase 5: Deploying Core Infrastructure"
...
################################################################################
# Phase 6: Complete Media Stack
################################################################################

deploy_complete_media_stack() {
    log_section "Phase 6: Deploying Complete Media Stack"
...
################################################################################
# Phase 7: AI & Development Stack
################################################################################

deploy_ai_development() {
    log_section "Phase 7: Deploying AI & Development Stack"
...
################################################################################
# Phase 8: Smart Home Stack
################################################################################

deploy_smart_home() {
    log_section "Phase 8: Deploying Smart Home Stack"
...
deploy_network_services() {
    log_section "Phase 9: Deploying Network Services"
...
EOF
    ...
    sleep 5
    RUSTDESK_KEY=$(docker logs hbbs 2>&1 | grep "Key:" | awk '{print $NF}')
    
    log_info "✓ Network services deployed"
    log_info "  RustDesk Key: $RUSTDESK_KEY"
}

################################################################################
# Phase 10: Complete Monitoring Stack
################################################################################

deploy_complete_monitoring() {
    log_section "Phase 10: Deploying Complete Monitoring Stack"
...
################################################################################
# Phase 11: Storage & Backup
################################################################################

deploy_storage_backup() {
    log_section "Phase 11: Deploying Storage & Backup Services"
...
################################################################################
# Phase 12: Security Stack
################################################################################

deploy_security() {
    log_section "Phase 12: Deploying Security Services"
...
################################################################################
# Phase 13: Dashboard Services
################################################################################

deploy_dashboards() {
    log_section "Phase 13: Deploying Dashboard Services"
...
deploy_utilities() {
    log_section "Phase 14: Deploying Utility Services"
...
print_complete_summary() {
    ...
    echo ""
    echo -e "${GREEN}═══════════════════════════════════════════════════════════════${NC}"
    echo -e "${GREEN}  🎉 ALL 60+ SERVICES DEPLOYED SUCCESSFULLY!${NC}"
    echo -e "${GREEN}═══════════════════════════════════════════════════════════════${NC}"
    echo ""
    ...
    echo -e "${YELLOW}═══════════════════════════════════════════════════════════════${NC}"
    echo ""
    echo "All credentials have been saved to:"
    echo "$HOMELAB_DIR/docs/CREDENTIALS.txt"
    echo ""
...
EOF
    ...
    echo "2. Delete the CREDENTIALS.txt file"
    ...
    log_info "Installation log saved to: $HOMELAB_DIR/docs/install-$(date +%Y%m%d-%H%M%S).log"
}
